<template>
  <div>
    ...
    <div class="contenedorCentralLog">
      ...
      <div class="contenedorLog">
        <form id="forma" name="forma" v-on:submit.prevent="processCerrarUrna">
          <div class="elemento">
            <label for="CodigoDeLaUrna">Código de la urna</label>
            <input
              type="text"
              id="CodigoDeLaUrna"
              v-model="closeUrna.codigoUrna"
              name="CodigoDeLaUrna"
              required="true"
            />
          </div>
...
export default {
  ...
  methods: {
    processCerrarUrna: async function() {
      if (
      ...
      await this.$apollo
        .mutate({
          mutation: gql`
            mutation CloseUrna($urnaCerrada: cerrarUrna!) {
              closeUrna(urnaCerrada: $urnaCerrada) {
                codigo
                nombre
                descripcion
                fecha
          ...
          `,
          variables: {
            urnaCerrada: this.closeUrna,
          },
        })
        .then((result) => {
          alert("Urna cerrrada con éxito");
          location.reload();
        })
        .catch((error) => {
          alert("Error al cerrar la urna");
        });
    },
    loadLogIn: function() {
